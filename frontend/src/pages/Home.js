...
const Home = ({ history, props, setSearchloading }) => {
  const boardlist = {
    ...
    "우리동네 동작구": "1728",
    "우리동네 강동구": "1733",
    "우리동네 김포/고양/일산/파주": "1740",
    "우리동네 성남/분당/광주": "1741",
    "우리동네 과천/안양/의왕/군포": "1742",
    "우리동네 광명/부천": "1743",
    "우리동네 시흥/안산": "1744",
    "우리동네 남양주/구리/하남": "1745",
    "우리동네 수원/화성/평택": "1746",
    "우리동네 양주/의정부/동두천": "1747",
    "우리동네 용인/오산": "1748",
    "우리동네 경기북부": "1749",
    "우리동네 경기남부": "1750",
    "우리동네 인천": "1751",
    ...
    "기타 컴퓨터용품": "393",
    "전자사전/MC스퀘어": "449",
    "대형가전(TV,냉장고,세탁기,에어컨 등)": "452",
    "소형가전(밥솥,다리미,가습기 등)": "451",
    "사무용품 복사기팩스": "341",
    "[명품]루이비통/샤넬": "353",
    "[명품]구찌/프라다": "788",
    "[명품]펜디/버버리": "784",
    ...
    "[도서]소설/만화책": "460",
    "[도서]전집/종교/기타": "459",
    "[부동산]강남·서초·강동·송파": "1678",
    "[부동산]금천·구로·강서·양천": "1680",
    "[부동산]마포·은평·서대문구": "1681",
    "[부동산]중구·종로·동대문구": "1682",
    "[부동산]성북·성동·광진·용산": "1683",
    "[부동산]중랑·강북·노원·도봉": "1684",
    "[부동산]동작·관악·영등포": "1679",
    "[부동산]경기(남부지역)": "1685",
    "[부동산]경기(북부지역)": "1702",
    "[부동산]인천광역시": "1686",
  ...
  const Search = () => {
    ...
    backend
      .get(`/search/${boardState}/${inputdata}/${page}`)
      .then((res) => {
    ...
    // 다음페이지 미리 로딩
    backend
      .get(`/search/${boardState}/${inputdata}/${page + 1}`)
      .then((res) => {
        setPage(page + 1);
    ...
    <div>
      <Container>
            ...
            <Form.Group controlId="exampleForm.SelectCustom">
              ...
              >
                {Object.keys(boardlist).map((one) => (
                  <option key={boardlist[one]} value={boardlist[one]}>
                    {one}
                  </option>
                ))}
              </Form.Control>
        ...
        <Row className="justify-content-center">
          <div
            style={{
              visibility: realdata.length > 0 ? "visible" : "hidden",
              textAlign: "center"
            }}
          >
